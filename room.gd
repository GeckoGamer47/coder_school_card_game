...
var first_room : RootMotionView


#var room_scene : PackedScene = preload("res://room_template.tscn")
...
func _ready ():
	...
	for x in range(map_size):
		...
		print(line)
	#there might be a problem with this code, or I saved the wrong thing as the root node. idk
...
func _check_room (x : int, y : int, desired_direction : Vector2, is_first_room : bool = false):
	...
	if x < 0 or x > map_size - 1 or y > map_size - 1:
		return
